...
if(!kanal) return message.channel.send(`Jail log kanalı ayarlı değil.`)
  ...
  if(!kişi) return message.channel.send('**Lütfen Hapisane Kişiyi Etiketleyiniz! Örnek;** `!jail @üye <süre> <sebep>` ')
  if(kişi.hasPermission("MANAGE_GUILD")) return message.channel.send(`Olmaz. Bu kişiyi susturamam.`)
  
  let zaman = args[1]
  if(!args[1]) return message.channel.send('**Ne Kadar Sürede Jailde Duracağını Belirtmelisin Örnek;** `!jail @üye <süre> <sebep>`')
